...
export function fetchFromSource(config, count = 10, options = {}) {
  ...
  if (source === "bookmarks") {
    ...
    throw new Error(
      `Invalid source: ${source}. Must be 'bookmarks', 'likes', or 'both'.`
    );
  }
}
...
export async function fetchArticleContent(url) {
  try {
    ...
    if (
      (result.includes("Subscribe") && result.includes("sign in")) ||
...
export async function fetchAndPrepareBookmarks(options = {}) {
  ...
  if (hasFolders && source === "bookmarks") {
    ...
    console.log(
      `Fetching from ${Object.keys(config.folders).length} folder(s)${
        includeMedia ? " (with media)" : ""
      }`
    );
    ...
    console.log(
      `Fetching from source: ${source}${includeMedia ? " (with media)" : ""}${
        fetchOptions.all ? " (paginated)" : ""
      }`
    );
  ...
  try {
    if (fs.existsSync(config.pendingFile)) {
      ...
      pendingIds = new Set(
        (pending.bookmarks || []).map((b) => b.id.toString())
      );
    }
  } catch (e) {}
  ...
  if (options.specificIds) {
    toProcess = tweets.filter((b) =>
      options.specificIds.includes(b.id.toString())
    );
  ...
  for (const bookmark of toProcess) {
    try {
      ...
      if (bookmark.createdAt) {
        const tweetDate = dayjs(bookmark.createdAt).tz(
      ...
      const author =
        bookmark.author?.username || bookmark.user?.screen_name || "unknown";
      ...
      for (const link of tcoLinks) {
        ...
        if (expanded.includes("github.com")) {
          ...
          expanded.includes("youtube.com") ||
          expanded.includes("youtu.be")
        ) {
          type = "video";
        } else if (
          ...
          if (expanded.includes("/photo/") || expanded.includes("/video/")) {
            ...
            if (tweetIdMatch) {
              ...
              console.log(
                `  Quote tweet detected, fetching ${quotedTweetId}...`
              );
              ...
              if (quotedTweet) {
                content = {
                  ...
                  tweetUrl: `https://x.com/${
                    quotedTweet.author?.username || "unknown"
                  }/status/${quotedTweet.id}`,
                  source: "quote-tweet",
                };
              }
            }
        ...
        if (type === "article" || type === "github") {
          try {
            ...
            if (fetchResult.source === "github-api") {
              ...
              console.log(
                `  GitHub repo: ${fetchResult.fullName} (${fetchResult.stars} stars)`
              );
            } else {
              content = {
      ...
      if (bookmark.inReplyToStatusId) {
        console.log(
          `  This is a reply to ${bookmark.inReplyToStatusId}, fetching parent...`
        );
        ...
        if (parentTweet) {
          replyContext = {
            ...
            authorName:
              parentTweet.author?.name ||
              parentTweet.author?.username ||
              "unknown",
            text: parentTweet.text || parentTweet.full_text || "",
            tweetUrl: `https://x.com/${
              parentTweet.author?.username || "unknown"
            }/status/${parentTweet.id}`,
          };
        }
      }
      ...
      if (bookmark.quotedTweet) {
        ...
        quoteContext = {
          ...
          text: qt.text || "",
          tweetUrl: `https://x.com/${qt.author?.username || "unknown"}/status/${
            qt.id
          }`,
          source: "native-quote",
        };
      ...
      console.log(
        `  Prepared: @${author} with ${
          links.length
        } links${mediaInfo}${tagInfo}${replyContext ? " (reply)" : ""}${
          quoteContext ? " (quote)" : ""
        }`
      );
    } catch (error) {
      console.error(
        `  Error processing bookmark ${bookmark.id}: ${error.message}`
      );
    }
  }
  ...
  const existingPendingIds = new Set(
    existingPending.bookmarks.map((b) => b.id)
  );
